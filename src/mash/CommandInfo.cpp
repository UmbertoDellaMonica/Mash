...
#define HASH "MurmurHash3_x64_128"
    ...
    int CommandInfo::writeJson(const Sketch &sketch) const
    {
        ...
        for (uint64_t i = 0; i < sketch.getReferenceCount(); i++)
        {
            ...
            {
                const vector<HashList> &refVectorHashList = ref.subSketch_list[j];

                cout << "        {" << endl;
                cout << "          \"Hashes\" : [" << endl;

                for (size_t k = 0; k < refVectorHashList.size(); k++)
                {
                    HashList hashList = refVectorHashList.at(k);
    ...
    int CommandInfo::writeFingerPrintJson(const SketchFingerPrint &sketch) const
    {
        ...
        for (uint64_t i = 0; i < sketch.getReferenceCount(); i++)
        {
            const SketchFingerPrint::Reference &ref = sketch.getReference(i);
            ...
            for (int j = 0; j < ref.subSketch_list.size(); j++)
            {
                HashList subsketch_hashlist = &ref.subSketch_list.at(j);
                ...
                for (size_t k = 0; k < subsketch_hashlist.size(); k++)
                {
                        ...
                        if (subsketch_hashlist.get64())
                        {
                            cout << "            " << subsketch_hashlist.at(j).hash64;
                        }
                        else
                        {
                            cout << "            " << subsketch_hashlist.at(j).hash32;
                        }
                }
